...
function VehicleInFront()
  local player = PlayerPedId()
    ...
    local entityWorld = GetOffsetFromEntityInWorldCoords(player, 0.0, 8.0, 0.0)
...
function StreachertoCar()
    ...
    if DoesEntityExist(closestObject) then
        if GetVehiclePedIsIn(ped, false) == 0 and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
            AttachEntityToEntity(closestObject, veh, 0.0, 0.0, -3.3, 1.0, 0.0, 0.0, 180.0, false, false, true, false, 2, true)
            FreezeEntityPosition(closestObject, true)
        else
            ESX.ShowNotification('Il n\'y a pas de ~r~véhicule~w~ pret de vous')--FR
           -- ESX.ShowNotification('There is no ~r~vehicle ~w~ near you')--EN
        end
    else
        ESX.ShowNotification('Tu est trop loin du ~r~véhicule~w~!')--FR
        -- ESX.ShowNotification('You are too far from the ~r~ vehicle ~w~!')--EN
    end
end
...
function StretcheroutCar()
    ...
    local closestObject = GetClosestObjectOfType(pedCoords, 6.0, GetHashKey("prop_ld_binbag_01"), false)
    if DoesEntityExist(closestObject) then
        if GetVehiclePedIsIn(playerPed, false) == 0 and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
            ...
            SetEntityCoords(closestObject, coords.x-1,coords.y-3,coords.z-0.8)
        end
    end
end
-----------------------------------------------------------------------------------------------------------------------
...
AddEventHandler('sit', function(strObject)
    ...
    if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
        if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), inBedDicts, inBedAnims, 3) then
            ESX.ShowNotification("Quelqu'un utilise déjà la civière!")--FR
           -- ESX.ShowNotification("Someone is already using the stretcher!")--EN
            return
        end
    end
  ...
  if IsPedDeadOrDying(playPed) then
    revivePed(playPed)
    AttachEntityToEntity(PlayerPedId(), strObject, 0, 0, 0.2, 1.0, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2, true)
...
function PickUp(strObject)
    ...
    if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
        if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2ig_11', 'pushcar_offcliff_f', 3) then
            ESX.ShowNotification("Tu pousse déjà la ~b~civière!")--FR
            --ESX.ShowNotification("Somebody is already pushing the Stretcher!")--EN
            return
        end
    end
    ...
    local pedid = PlayerPedId() ------------------------------------------------------disdetoi-hauteur-
    AttachEntityToEntity(strObject, pedid, GetPedBoneIndex(PlayerPedId(), 28422), -0.45, -1.3, 0.1, 180.0, 180.0, 180.0, 0.0, false, false, true, false, 2, true)
    while IsEntityAttachedToEntity(strObject, pedid) do
        Citizen.Wait(5)

        if not IsEntityPlayingAnim(pedid, 'missfinale_c2ig_11', 'pushcar_offcliff_f', 3) then
            TaskPlayAnim(pedid, 'missfinale_c2ig_11', 'pushcar_offcliff_f', 8.0, 8.0, -1, 50, 0, false, false, false)
        end
